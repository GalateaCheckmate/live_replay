...
pub async fn get_replay_sessions(
    ...
    let rows = sqlx::query(
        "SELECT s.id, s.live_streamer_id, s.streamer_name, s.streamer_url, \
                s.live_title, s.started_at, s.ended_at, s.status, s.aid, s.bvid, \
                s.expected_parts, s.verified_parts, s.last_error, \
                (SELECT COUNT(*) FROM recording_segments r \
                 WHERE r.session_id = s.id AND r.status NOT IN ('verified', 'deleted')) AS pending_parts \
...
pub async fn get_replay_jobs(
    ...
    let rows = sqlx::query(
        "SELECT j.id, r.session_id, r.id AS segment_id, s.streamer_name, s.bvid, \
                r.part_number, r.file_path, r.file_size, r.status AS segment_status, \
         ...
         ORDER BY CASE WHEN j.status IN ('queued', 'uploading', 'retry_wait') THEN 0 ELSE 1 END, \
...
pub async fn retry_replay_job(
    ...
    let segment_id: Option<i64> = sqlx::query_scalar("SELECT segment_id FROM upload_jobs WHERE id = ?")
        .bind(id)
    ...
    sqlx::query(
        "UPDATE recording_segments SET status = 'queued', next_retry_at = NULL, \
         ...
         AND status NOT IN ('verified', 'deleted')",
    )
    .bind(segment_id)
    .execute(&mut *tx)
    .await
    ...
    sqlx::query(
        "UPDATE upload_jobs SET status = 'queued', next_attempt_at = NULL, \
         last_error = NULL, locked_at = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ? \
         AND status NOT IN ('verified', 'complete')",
    )
    .bind(id)
    .execute(&mut *tx)
    .await
