...
function App() {
  ...
  async function refreshCoreStatus() {
    try {
      ...
      else if (next.active) setMessage(`正在录制：${next.display_name || "直播"}`);
  ...
  async function probeStream() {
    ...
    try {
      ...
      setMessage(
        result.status === "live"
          ? `已解析 ${result.stream.platform}：${result.stream.title || result.stream.name}`
          : "主播当前未开播",
      );
    } catch (error) {
      setMessage(String(error));
  ...
  async function addMonitor() {
    ...
    setBusy(true);
    try {
      const next = await invoke<MonitorStore>("mobile_monitor_add", {
        url: url.trim(),
        name: name.trim() || null,
      });
      setMonitor(next);
      setMessage("已加入监控。以后检测到开播会自动开始整场录制。")
      setUrl("");
      setName("");
      setProbe(null);
    } catch (error) {
  ...
  async function toggleMonitor(target: MonitorTarget) {
    try {
      setMonitor(
        await invoke<MonitorStore>("mobile_monitor_set_enabled", {
          targetId: target.id,
  ...
  async function removeMonitor(target: MonitorTarget) {
    try {
      setMonitor(
        await invoke<MonitorStore>("mobile_monitor_remove", { targetId: target.id }),
      );
    } catch (error) {
      setMessage(String(error));
    }
  ...
  async function stopRecording() {
    setBusy(true);
    setMessage("正在停止录制并收尾完整录像...");
    try {
      setCore(await invoke<CoreStatus>("mobile_stop_recording"));
  ...
  async function updateYoutubeSettings(autoUpload: boolean, deleteAfterSuccess: boolean) {
    try {
      const store = await invoke<YoutubeStore>("mobile_youtube_set_settings", {
        autoUpload,
        deleteAfterSuccess,
      });
  ...
  async function retryUpload(task: UploadTask) {
    try {
      const store = await invoke<YoutubeStore>("mobile_youtube_retry_task", {
        taskId: task.id,
      });
      setYoutube((current) => ({ ...current, store }));
  ...
  const pendingUploads = youtube.store.tasks.filter(
    (task) => task.state !== "UPLOAD_SUCCESS",
  ).length;
  const successfulUploads = youtube.store.tasks.filter(
    (task) => task.state === "UPLOAD_SUCCESS",
  );
  ...
  return (
    <main className="app-shell">
      ...
      {tab === "record" && (
        <>
          <section className="stats-grid" aria-label="任务概览">
            ...
            <article><span>录制中</span><strong>{core.active ? 1 : 0}</strong></article>
            <article><span>待上传</span><strong>{pendingUploads}</strong></article>
          </section>

          <section className="panel recorder-panel">
            <div className="section-heading">
              <div><h2>添加主播</h2><p>检测到开播后自动整场录制</p></div>
              <span className="status-pill">自动监控</span>
            </div>
            <label className="field">
              <span>直播间地址</span>
              <input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="B站或抖音直播间地址" />
            </label>
            <label className="field">
              <span>主播名称</span>
              <input value={name} onChange={(event) => setName(event.target.value)} placeholder="例如：昊天" />
            </label>
          ...
          {probe?.status === "live" && (
            <section className="panel result-panel">
              <div className="section-heading">
                <div><h2>{probe.stream.title || probe.stream.name}</h2><p>{probe.stream.platform} · {probe.stream.suffix}</p></div>
          ...
          <section className="panel">
            <div className="section-heading"><div><h2>监控列表</h2><p>状态会在后台自动刷新</p></div></div>
              ...
              {monitor.targets.map((target) => (
                <article className="task-item" key={target.id}>
                  <div className="task-main">
                    ...
                    <span>{target.last_error || target.last_state}</span>
          ...
          {core.active && (
            <button className="danger-action full-action" type="button" onClick={stopRecording} disabled={busy}>停止当前录制</button>
          )}
        </>
      )}
      ...
      {tab === "upload" && (
        <section className="panel">
          <div className="section-heading">
            <div><h2>YouTube 上传队列</h2><p>Private · Resumable Upload</p></div>
            <span className="status-pill">{youtube.authorized ? "已登录" : "未登录"}</span>
          </div>
          <div className="task-list">
            ...
            {youtube.store.tasks.slice().reverse().map((task) => {
              ...
              return (
                <article className="task-item stacked" key={task.id}>
                  ...
                  {task.state !== "UPLOAD_SUCCESS" && task.state !== "UPLOAD_RESULT_UNKNOWN" && (
                    <button className="mini-button" type="button" onClick={() => retryUpload(task)}>重试</button>
                  )}
                </article>
              );
            })}
      ...
      {tab === "history" && (
        <>
          <section className="panel compact-panel">
            <div className="section-heading"><div><h2>最近完整录像</h2><p className="path-text">{core.last_file || "暂无"}</p></div></div>
          </section>
          <section className="panel">
            ...
            <div className="task-list">
              ...
              {successfulUploads.slice().reverse().map((task) => (
                <article className="task-item" key={task.id}>
                  <div className="task-main">
                    <strong>{task.youtube_title}</strong>
                    <span>{task.local_deleted ? "YouTube 成功 · 本地已安全删除" : "YouTube 成功 · 本地保留"}</span>
                    {task.youtube_video_id && <small>videoId: {task.youtube_video_id}</small>}
                  </div>
                </article>
              ))}
        ...
        <>
          <section className="panel">
            ...
            <div className="settings-actions">
              {!youtube.authorized ? (
                <button className="primary-action" type="button" onClick={authorizeYoutube} disabled={busy}>登录 YouTube</button>
              ) : (
                <button className="secondary-action" type="button" onClick={logoutYoutube} disabled={busy}>退出账号</button>
              )}
            </div>
          </section>
          ...
          <section className="panel settings-list">
            <label className="setting-row">
              <div><strong>自动上传</strong><span>整场录像结束后自动上传 YouTube Private</span></div>
              <input type="checkbox" checked={youtube.store.settings.auto_upload} onChange={(event) => updateYoutubeSettings(event.target.checked, youtube.store.settings.delete_after_success)} />
            </label>
            <label className="setting-row">
              <div><strong>上传成功后删除本地</strong><span>仅在 videoId 与成功状态持久化后删除</span></div>
              <input type="checkbox" checked={youtube.store.settings.delete_after_success} onChange={(event) => updateYoutubeSettings(youtube.store.settings.auto_upload, event.target.checked)} />
            </label>
            <div className="setting-row static"><div><strong>默认可见性</strong><span>第一阶段固定为 Private</span></div><b>Private</b></div>
          </section>

          <section className="panel compact-panel">
            <div className="section-heading"><div><h2>后台运行</h2><p>Foreground Service + WakeLock · 录制与上传独立运行</p></div><span className="status-pill">已启用</span></div>
          </section>
        </>
      )}
